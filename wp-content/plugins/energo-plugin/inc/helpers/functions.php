...
//Pagination
function energo_the_pagination2($args = array(), $echo = 1)
{
...
add_filter('energo_redux_custom_fonts_load', 'energo_redux_custom_fonts_load');
function energo_redux_custom_fonts_load( $custom_font ) {
	$custom_style = '';
	...
	if ( $filename = energo_set( $pathinfo, 'filename' ) ) {
		...
		foreach ( $extensions as $extension ) {
			$file_path = esc_url(home_url('/')) . '/wp-content/themes/energo/assets/css/custom-fonts/' . $filename . '.' . $extension;
...
/**
 * [energo_social_share_output description]
 *
 ...
 */
function energo_social_share_output( $icon, $color = false ) {
...
add_action('energo_social_share_output', 'energo_social_share_output');

function energo_share_us($PostID = '', $PostName = '')
{
$options = energo_WSH()->option();
?>
<ul class="share-links clearfix">
	<?php if($options->get( 'facebook_sharing' )):?>
	<li><a href="http://www.facebook.com/sharer.php?u=<?php echo esc_url(get_permalink($PostID)); ?>" target="_blank"><i class="fab fa-facebook-f"></i><?php esc_html_e('Facebook', 'energo'); ?></a></li>
	<?php endif;?>

	<?php if($options->get( 'twitter_sharing' )):?>
	<li><a href="https://twitter.com/share?url=<?php echo esc_url(get_permalink($PostID)); ?>&text=<?php echo esc_attr($post_slug=$PostName); ?>" target="_blank"><i class="fab fa-twitter"></i><?php esc_html_e('Twitter', 'energo'); ?></a></li>
	<?php endif;?>

	<?php if($options->get( 'linkedin_sharing' )):?>
	<li><a href="http://www.linkedin.com/shareArticle?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="fab fa-linkedin"></i><?php esc_html_e('Linkedin', 'energo'); ?></a></li>
	<?php endif;?>

	<?php if($options->get( 'pinterest_sharing' )):?>
	<li><a href="https://pinterest.com/pin/create/bookmarklet/?url=<?php echo esc_url(get_permalink($PostID)); ?>&description=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="fab fa-pinterest"></i><?php esc_html_e('Pinterest', 'energo'); ?></a></li>
	<?php endif;?>

	<?php if($options->get( 'reddit_sharing' )):?>
	<li><a href="http://reddit.com/submit?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="fab fa-reddit"></i><?php esc_html_e('Reddit', 'energo'); ?></a></li>
	<?php endif;?>

	<?php if($options->get( 'tumblr_sharing' )):?>
	<li><a href="http://www.tumblr.com/share/link?url=<?php echo esc_url(get_permalink($PostID)); ?>&name=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="fab fa-tumblr"></i><?php esc_html_e('Tumblr', 'energo'); ?></a></li>
	<?php endif;?>

	<?php if($options->get( 'digg_sharing' )):?>
	<li><a href="http://digg.com/submit?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="fab fa-digg"></i><?php esc_html_e('Digg', 'energo'); ?></a></li>
	<?php endif;?>
</ul>
<?php }

function energo_get_social_icons()
{
	$options = energo_WSH()->option(); ?>
    <?php $icons = $options->get( 'icons_social_share' );
		...
		<ul class="social-links clearfix">
			...
			<li><a href="<?php echo esc_url(energo_set( $header_social_icons, 'url' )); ?>" style="background-color:<?php echo esc_attr(energo_set( $header_social_icons, 'background' )); ?>; color: <?php echo esc_attr(energo_set( $header_social_icons, 'color' )); ?>"><i class="fab <?php echo esc_attr( energo_set( $header_social_icons, 'icon' ) ); ?>"></i></a></li>
			<?php endforeach; ?>
		</ul>
	<?php endif; ?>
<?php }

function energo_get_social_icons2()
{
	$options = energo_WSH()->option();
	?>
